...
---
绘制等值线地图，第二部分：添加交互
在[第1部分] (/help/tutorials/choropleth-studio-gl-pt-1)中，您在Mapbox Studio样式编辑器中设置了美国人口密度数据的样式，并发布了一个新样式。在第2部分中，您将使用Mapbox GL JS使这种样式在交互中变得生动起来。
{{
...
在[MapBox样式规范] (https://www.mapbox.com/mapbox-gl-style-spec/)中[地图样式] (/help/glossary/style)是一个JSON对象，它包含浏览器正确绘制地图所需的所有内容。它的主要部分是：
...
##开始
以下是您需要开始的内容：
- [**An access token**](/help/glossary/access-token)。您可以在您的[帐户页](https://www.mapbox.com/account/)上找到访问令牌。
- 您的样式的 [**style URL**](/help/glossary/style-url) 。 在您的[样式](https://www.mapbox.com/studio) 页面, 点击人口密度样式旁边的**菜单{{<Icon name='menu' inline={true} />}}**按钮，然后点击{{<Icon name='clipboard' inline={true} />}}剪贴板图标复制**样式URL **。
- [**Mapbox GL JS**](https://www.mapbox.com/mapbox-gl-js). 用于构建网页地图的Mapbox JavaScript API。
- **A text editor**.您毕竟要编写HTML,CSS, 和JavaScript。
##创建网页
打开文本编辑器并创建一个名为`index.html`的文件。通过在头部添加Mapbox GL JS及其关联的CSS文件来配置文档：
```html
...
```
您还需要应用一些CSS来可视化布局的外观。这对于`map` div尤其重要，除非您给它一个`height`，否则它不会显示在页面上：
```css
body {
  margin: 0;
  padding: 0;
...
##初始化地图
现在您已经向页面添加了结构，就可以开始编写一些JavaScript 了！首先需要做的是添加访问令牌。没有这个，剩下的代码就不能生效。_注意：以下所有代码都应该在脚本标记之间。_
```js
...
现在您已经添加了页面的结构，您可以将一个Map对象添加到`map` div中。请确保将`你的样式url`替换为您在本指南第1部分中创建的样式的[样式 URL](/help/glossary/style-url)，否则代码不会生效！
```js
...
<p>在页面上初始化地图不仅仅是在<code>map</code>div中创建一个容器。它还告知浏览器请求你在第1部分中创建的Mapbox Studio样式。根据Mapbox服务器对该请求的响应速度，这可能需要花费不确定的时间，而代码中要添加的所有内容都依赖于加载到地图中的样式。因此，确保在执行更多代码之前加载样式非常重要。</p><p>幸运的是，地图对象可以告诉浏览器当地图的状态改变时发生的某些事件。其中一个事件是<code>load</code>，当样式加载到地图上时会触发该事件。通过<code>map.on</code>方法，您可以将事件放在一个<a href='https://github.com/maxogden/art-of-node#callbacks'>回调函数</a>中（在<code>load</code>事件发生时调用该函数），确保在该事件发生之前不会执行剩下的代码</p>。
</Note>
}}
...
为了确保剩下的代码能够执行，它需要停留在一个*回调函数*中，该函数在地图加载完成时执行。
```js
...
要执行此操作，请为`mousemove`事件添加一个监听器，确定光标所在位置的州（如果有的话），然后更新信息窗体：
...
干得好！有关Mapbox Studio的更多操作，请参阅[Mapbox Studio Manual](https://www.mapbox.com/studio-manual/)。有关Mapbox GL JS及其工作方式的更多信息，请阅读[How web apps work](/help/how-mapbox-works/web-apps/) 指南。
